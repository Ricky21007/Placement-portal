...
import { collection, addDoc, Timestamp, serverTimestamp, updateDoc, doc, setDoc } from 'firebase/firestore';
...
const ScheduleInterview: React.FC<ScheduleInterviewProps> = ({ application, onClose }) => {
  ...
  const [interview, setInterview] = useState<Interview | null>(null); // holds scheduled interview for outcome buttons
    ...
    <form onSubmit={handleSubmit} style={{ marginTop: 12, background: '#f8f8ff', padding: 16, borderRadius: 8 }}>
      ...
      {interview?.status === "Scheduled" && (
        <div style={{ marginTop: 16 }}>
          <button
            onClick={() =>
              markOutcome(interview.id, "Hired", interview.graduateId, interview.jobId, interview.jobTitle)
            }>
            Mark as Hired
          </button>
          <button
            onClick={() =>
              markOutcome(interview.id, "Not Hired", interview.graduateId, interview.jobId, interview.jobTitle)
            }
            style={{ marginLeft: 10 }}>
            Not Hired
          </button>
